...
const Timer = props => {
  const { id, name, finalDate, createdAt, warnDate, itemClass, openEditor, closeEditor } = props;
  ...
  const renderWarning = () => {
    return (<>
      {
        isWarning &&
        <span className={styles.warn} title={warnDate}></span>
      }
    </>)
  }
  ...
  return (
    <li className={listStyles} title={'Edit current timer'} onClick={openEditorHandler}>
      {
        renderWarning()
      }
