...
const CreatorFilter = (props) => {
  ...
  const changePredicate = ({ name, value }) => {
    if (name === "types") {
      ...
      contestsTypes.has(value)
        ? contestsTypes.delete(value)
        : contestsTypes.add(value)
      const newTypes = [...contestsTypes]
      parseParamsToUrl({ ...creatorFilter, types: newTypes })
    } else {
      parseParamsToUrl({
        ...creatorFilter,
        ...{ [name]: value === "All industries" ? "" : value },
      })
    }
  }
  ...
  const parseUrlForParams = (search) => {
    ...
    const filter = {
      types:
        parsedParams.types && parsedParams.types.length > 0
          ? parsedParams.types
          : CONSTANTS.CONTEST_TYPES,
      ...
      {!isFetching && (
        ...
        >
          ...
          {data.industry &&
            ...
            data.industry.map((indstr) => (
              <option key={indstr} value={indstr}>
                {indstr}
              </option>
            ))}
        </CreatorFilterItem>
